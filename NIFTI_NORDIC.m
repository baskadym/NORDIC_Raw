...
if isfield(ARG,'use_generic_NII_read') % save out a map of a relative gfactor
    if ARG.use_generic_NII_read==1
        path(path,'/home/mr/github/baskadym/ASPIRE/dependencies/');
    end
else
...
if ( ARG.save_gfactor_map==2 )  | ( ARG.save_gfactor_map==1 )
    ...
    elseif strmatch(info.Datatype,'int32')
        g_IMG= int32(abs(g_IMG)*2^gain_level);
    else
...
if isfield(ARG,'make_complex_nii')
    ...
    elseif strmatch(info.Datatype,'int16')
        IMG2_tmp= int16(abs(IMG2_tmp)*2^gain_level);
    elseif strmatch(info.Datatype,'int32')
        IMG2_tmp= int32(abs(IMG2_tmp)*2^gain_level);
    else
    ...
    elseif strmatch(info_phase.Datatype,'int32')
        IMG2_tmp= int32(IMG2_tmp);
    else
    ...
    if 0
        ...
        elseif strmatch(info_phase.Datatype,'int32')
            IMG2_tmp=IMG2_tmp/(2*pi)*phase_range;
            IMG2_tmp= int32((IMG2_tmp)*2^gain_level);
        else
            IMG2_tmp= single(abs(IMG2_tmp)*2^gain_level);
        end
    end
    ...
    elseif strmatch(info.Datatype,'int32')
        IMG2= int32(abs(IMG2)*2^gain_level);
    else
